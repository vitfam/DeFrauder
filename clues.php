                  ...
                  while($row1 = mysqli_fetch_assoc($res1)){
                    ...
                    echo '
                      <div class="container story-box my-4">
                        ...
                        <p><a href="' . $row1['clue_content'] . '" target="_blank">Click here to Access</a></p>
                      ...
                      ';
                        if ($id == 1) {
                          ...
                          <form action="' . $_SERVER['PHP_SELF'] . '" method="POST">
                            <input type="submit" class="btn btn-secondary disabled my-2 mx-2" name="prev" value="Previous"/>
                          </form>
                          ';
                        }
                        else{
                        ...
                        if($id == 15){
                          if($clue_row['ques_increment'] == 4){
                            echo '
                              <a href="./final.php" role="submit" class="btn btn-success my-2 mx-2">Final Question</a>
                            ';
                          }else{
                            echo '
      ...
      if (mysqli_num_rows($clueListResult)) {
        while($clueListRow = mysqli_fetch_assoc($clueListResult)){
          if($clueListRow['ques_increment'] > 3){
            for ($i = 1; $i <= 15; $i++) { 
              echo '<button type="button" class="btn btn-outline-primary btn-sm mx-1 my-2" data-bs-toggle="modal" data-bs-target="#clueModal' . $i . '">Clue ' . $i . '</button>';
              require './partials/_clueModal.php';
              
              if($i == 15){
                echo '<button type="button" class="btn btn-dark btn-sm mx-1 my-2" data-bs-toggle="modal" data-bs-target="#stageModal3">Stage 3 Answer</button>';
                require './partials/_stageModal.php';
              }
              if($i == 10){
                echo '<button type="button" class="btn btn-dark btn-sm mx-1 my-2" data-bs-toggle="modal" data-bs-target="#stageModal2">Stage 2 Answer</button>';
                require './partials/_stageModal.php';
              }
              if($i == 5){
                echo '<button type="button" class="btn btn-dark btn-sm mx-1 my-2" data-bs-toggle="modal" data-bs-target="#stageModal1">Stage 1 Answer</button>';
                require './partials/_stageModal.php';
              }
            }
            ...
            for ($i = 1; $i <= 15; $i++) { 
              if($i <= 10){
                ...
                echo '<button type="button" class="btn btn-secondary btn-sm mx-1 my-2" disabled data-bs-toggle="modal" data-bs-target="#clueModal' . $i . '">Clue ' . $i . '</button>';
                require './partials/_clueModal.php';
              }
              if($i == 5){
                echo '<button type="button" class="btn btn-dark btn-sm mx-1 my-2" data-bs-toggle="modal" data-bs-target="#stageModal1">Stage 1 Answer</button>';
